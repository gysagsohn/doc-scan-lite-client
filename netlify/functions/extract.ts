...
const withCors = (statusCode: number, body: any) => ({
  statusCode,
  headers: {
...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    const baseMsg = { type: "text" as const, text: userPrompt(JSON.stringify(schema)) };
    const completion1 = await openai.chat.completions.create({
      model: MODEL,
      messages: [
        { role: "system", content: systemPrompt },
        { role: "user", content: [baseMsg, ...inputImgs] as any },
      ],
      temperature: 0.2,
    ...
    const iso = (s: any) =>
      s && /^\d{4}-\d{2}-\d{2}$/.test(s) ? s : s ? new Date(s).toISOString().slice(0, 10) : null;
